...
function goOn() {
  if (!currentTetrominos.goOneDown()) {
    ...
    // GAME OVER
    if (
      !currentTetrominos.isPositionValid(
        currentTetrominos.matrix,
        currentTetrominos.x,
        currentTetrominos.y + 1
      )
    ) {
      gameOver = true;
      alert("Game Over !");
...
function switchTetrominos() {
  if (
    ...
    nextTetrominos.isPositionValid(
      nextTetrominos.matrix,
      currentTetrominos.x,
      currentTetrominos.y
    ) &&
    ...
    [currentTetrominos.x, currentTetrominos.y] = [
      nextTetrominos.x,
      nextTetrominos.y,
    ];
  }
...
function submitForm(e) {
  if (pseudo.value === "") {
    ...
    alert(pseudo.value + " : Your score of " + bestScore + " has been sent !");
    form.submit();
  }
}
